...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(hidden_size, bias=True)
        ...
        self.target_vocab_projectionq = nn.Linear (hidden_size, len(self.vocab.tgt),bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        dec_hidden_state = self.h_projection(torch.cat(last_hidden))
        dec_cell_state = self.c_projection(torch.cat(last_cell))
        dec_init_state = (dec_hidden_state, dec_cell_state)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.target(target_padded)

        for Y_t in torch.split(Y):
            Ybar_t = torch.cat((torch.squeeze(Y_t),o_prev),-1)
            o_t = self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(torch.transpose(dec_hidden),enc_hiddens_proj)
