...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        last, secondlast = self.stack.pop(), self.stack.pop()
        print(self.stack)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        ...
        for sentence in minibatch:
            while len(sentence.buffer) != 0 or len(sentence.stack) != 1:
                transition = model.predict([sentence])
                sentence.parse(transition)
                print(transition,sentence.dependencies,sentence.buffer,sentence.stack)
