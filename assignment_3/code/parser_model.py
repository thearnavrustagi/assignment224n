...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        def xavier_and_parameter (xavier = True,n_features=(hidden_size,)):
            if xavier:
                return nn.Parameter(nn.init.xavier_uniform_(torch.zeros(n_features)))
            else:
                return nn.Parameter(nn.init.uniform_(torch.zeros(n_features)))

        self.embed_to_hidden_weight = xavier_and_parameter(n_features = (n_features*self.embed_size,hidden_size))
        self.embed_to_hidden_bias = xavier_and_parameter(False)

        self.hidden_to_logits_weight = xavier_and_parameter(n_features = (hidden_size,n_classes))
        self.hidden_to_logits_bias = xavier_and_parameter(False, n_features = (n_classes,))
    ...
    def embedding_lookup(self, w):
        ...
        x = np.array([[self.embeddings[word].detach().cpu().numpy() for word in line] for line in w])
        x = torch.tensor(x)
    ...
    def forward(self, w):
        ...
        w = torch.reshape(self.embedding_lookup(w),(w.shape[0],self.n_features*self.embed_size))
        z = torch.matmul(w, self.embed_to_hidden_weight) + self.embed_to_hidden_bias.repeat(w.shape[0],1)
        z = nn.ReLU()(z)
        z = torch.matmul(z,self.hidden_to_logits_weight) + self.hidden_to_logits_bias.repeat(w.shape[0],1)

        logits = self.dropout(z)
